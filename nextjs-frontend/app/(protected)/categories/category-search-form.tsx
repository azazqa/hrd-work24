...
export type CategorySearchInitial = Partial<FormState>;

function fromInitial(initial: CategorySearchInitial): FormState {
  return {
    name: initial.name ?? "",
...
export function CategorySearchForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/categories?${buildSearchQueryString(form, size)}`);
  };

  const handleReset = () => {
    setForm({ name: "", description: "" });
    router.push(`/categories?page=1&size=${size}`);
  };
